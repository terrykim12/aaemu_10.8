...
// 10.0.2.13 SC_PACKET_UNIT_STATE (opcode 151). Field order/types follow the reference
// UnitState_Serialize. FIRST-PASS v10 structure: the
...
#pragma warning restore IDE0052 // Remove unread private members
    ...
    public override PacketStream Write(PacketStream stream)
    {
        ...
        switch (_baseUnitType)
        {
            ...
            case BaseUnitType.Slave:
                ...
                stream.Write(slave.TlId);                 // tl (TODO: single-quant)
        ...
        // 12. second level/heirLevel pair (UnitState_SerializeLevelBlock)
        stream.Write(_unit.Level);
        stream.Write((byte)0); // heirLevel (TODO)
        // 13. slot array (UnitState_SerializeSlotArray4): 4 × u8. TODO(v10): real values
        stream.Write((byte)0);
        stream.Write((byte)0);
        stream.Write((byte)0);
        stream.Write((byte)0);
        ...
        // writes 3 signed bytes (rot.x/y/z) — the idType==3 branch of UnitState_Serialize against the rest.
        ...
        if (character is not null)
        {
            ...
            // pad to 29 entries if the model holds fewer (the reference always serializes 29 fixed slots)
            ...
            // slotInfoList = u32 Size + Size×{key u32, level u8, exp u32};
            // levelEffectList = u32 Size + Size×{equipSlot u8, level u8, type u32}.
    ...
    #region NetBuff
    // 10.0.2.13 buff record (UnitState_SerializeBuff): index group{u32} + SkillCaster +
